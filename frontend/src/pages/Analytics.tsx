  ...
  return (
    <div className="px-8 py-8 max-w-6xl">
      ...
      <div className="mt-6 bg-white rounded-xl border border-gray-100 px-6 py-5">
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-sm font-semibold text-gray-900">Revenue Impact Projector</h2>
            <p className="text-xs text-gray-400 mt-0.5">
              Estimate retained annual revenue from proactive outreach to high-risk customers
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-6 mb-5">
          <div className="col-span-2">
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-semibold text-gray-500">Assumed contact success rate</label>
              <span className="text-sm font-bold text-gray-900 tabular-nums">{retentionRate}%</span>
            </div>
            <input
              type="range"
            ...
            <div className="mt-5 bg-gray-50 rounded-lg px-4 py-3">
              <div className="grid grid-cols-3 gap-3">
                {[
                  { label: 'High-risk customers', value: `${data.risk_segments.high.count}` },
                  { label: 'Annual value at risk', value: `$${(data.risk_segments.high.annual_value_at_risk / 1000).toFixed(0)}k` },
                  { label: `Retained at ${retentionRate}%`, value: `$${((data.risk_segments.high.annual_value_at_risk * retentionRate) / 100 / 1000).toFixed(0)}k`, accent: true },
                ].map(item => (
                  <div key={item.label}>
                    <p className="text-[10px] text-gray-400 mb-0.5">{item.label}</p>
                    <p className={`text-sm font-bold ${item.accent ? 'text-accent' : 'text-gray-900'}`}>{item.value}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex flex-col items-center justify-center bg-accent/5 rounded-xl border border-accent/10 px-5 py-6 text-center">
            <p className="text-[10px] font-semibold uppercase tracking-wide text-accent mb-2">Est. Retained Revenue</p>
            <p className="text-4xl font-bold text-gray-900 tabular-nums leading-none">
              ${((data.risk_segments.high.annual_value_at_risk * retentionRate) / 100 / 1000).toFixed(0)}k
            </p>
            <p className="text-[10px] text-gray-400 mt-2 leading-snug">
              annual · from {data.risk_segments.high.count} high-risk customers contacted
            </p>
          </div>
        </div>

        <p className="text-[10px] text-gray-400 leading-snug border-t border-gray-50 pt-3">
          <span className="font-semibold text-gray-500">Projection note — </span>
          Success rate reflects the % of contacted high-risk customers expected to remain primary Capital One customers. Industry back-testing suggests 35–50% retention for proactive life-event outreach vs. 12% for reactive. Actual results depend on RM execution and product fit.
        </p>
      </div>
    </div>
  )
